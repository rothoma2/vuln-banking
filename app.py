...
swaggerui_blueprint = get_swaggerui_blueprint(
    ...
    API_URL,
    config={
        'app_name': "Northstar Bank API Documentation",
        'validatorUrl': None
    }
)
...
def metadata_hostname():
    if not _is_loopback_request():
        return make_response('Forbidden', 403)
    return make_response('northstar.internal\n', 200)
...
def metadata_iam_list():
    if not _is_loopback_request():
        return make_response('Forbidden', 403)
    return make_response('northstar-role\n', 200)

@app.route('/latest/meta-data/iam/security-credentials/northstar-role', methods=['GET'])
def metadata_iam_role():
    ...
    creds = {
        ...
        'RoleArn': 'arn:aws:iam::123456789012:role/northstar-role'
    }
...
def ai_chat_anonymous():
    """AI chat endpoint without authentication."""
    try:
        data = request.get_json()
        user_message = data.get('message', '')
...
def ai_system_info():
    ...
    try:
        return jsonify({
            ...
            'modes': {
                'authenticated': 'Requires JWT token and includes user context',
                'anonymous': 'No authentication and no user context'
            },
            'example_requests': [
