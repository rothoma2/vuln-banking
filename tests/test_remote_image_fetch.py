...
class RemoteImageFetchTests(unittest.TestCase):
    def test_download_public_image_accepts_public_url(self):
        with patch.dict(REMOTE_FETCH, {
            'resolve_public_ips': lambda hostname, port: ['93.184.216.34'],
            'fetch_public_image': lambda parsed, resolved_ips: (200, b'image-bytes'),
        }):
    ...
    def test_download_public_image_rejects_private_destination(self):
        with patch.dict(REMOTE_FETCH, {'resolve_public_ips': lambda hostname, port: []}):
    ...
    def test_download_public_image_rejects_redirects(self):
        with patch.dict(REMOTE_FETCH, {
            'resolve_public_ips': lambda hostname, port: ['93.184.216.34'],
            'fetch_public_image': lambda parsed, resolved_ips: (302, b''),
        }):
    ...
    def test_resolve_public_ips_prefers_global_addresses(self):
        with patch.object(REMOTE_FETCH['socket'], 'getaddrinfo', return_value=[
            (socket.AF_INET, socket.SOCK_STREAM, 6, '', ('10.0.0.1', 443)),
            (socket.AF_INET, socket.SOCK_STREAM, 6, '', ('93.184.216.34', 443)),
        ]):
            resolved_ips = REMOTE_FETCH['resolve_public_ips']('example.com', 443)

        self.assertEqual(resolved_ips, ['93.184.216.34'])

    def test_resolve_public_ips_handle_scoped_ipv6_addresses(self):
        with patch.object(REMOTE_FETCH['socket'], 'getaddrinfo', return_value=[
        ...
        self.assertEqual(resolved_ips, ['2606:2800:220:1:248:1893:25c8:1946'])
